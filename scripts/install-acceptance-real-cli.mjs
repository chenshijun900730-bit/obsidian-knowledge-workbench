...
const FAILURE_PREFIX = "Real-vault acceptance installation failed";
const SUCCESS_MESSAGE = "Installed the bound read-only acceptance build into the authorized real vault; Obsidian was not started.\n";
...
async function main() {
  ...
  try {
    ...
    const { installRealVaultAcceptance } = await import("./install-acceptance-real.mjs");
    ...
    await installRealVaultAcceptance({ repoRoot, ...request });
