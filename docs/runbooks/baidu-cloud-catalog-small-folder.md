...
- 文件夹浏览真实验收状态：本自动计划未执行；安装、启动、真实请求、备份或恢复都需要后续新的明确授权
...
5. 本节直接扫描的最终目录不能是 `/`，也不能为了寻找目标目录先枚举父目录。文件夹浏览器只可在第 9 节获得新授权后，把 `/` 当作一次会话披露保护的内部入口；运行时仍拒绝选择 `/` 或把它传给扫描和核验。
...
- 下载字节数不为零；
- 用户试图选择根目录、把根目录传给扫描或核验、绕过会话披露枚举根层、自动枚举父层或未完成最终确认；
- SecretStorage 属性尚未被接受；
- 插件运行在真实 Vault 中。

权限或配额错误只记录固定错误码。不得改用 Cookie、账号密码、隐藏接口、第三方工具、未经披露的父目录或根目录浏览、递归网盘扫描或并发请求绕过。
...
## 9. 后续文件夹浏览真实验收（本自动计划未执行）

本自动计划没有安装插件、打开 Obsidian、发起 OAuth 或百度列表请求，也没有执行本节。只有取得覆盖本节具体目标、Vault 和恢复动作的全新明确授权后，才可按顺序执行；任何旧授权、自动化通过或构建产物都不能替代该授权。

1. 在 Obsidian 完全退出且插件禁用时，把目标插件的原 `data.json`（如存在）和原 `community-plugins.json` 作为不解析的字节副本备份到用户控制的本机临时位置；不要把内容、路径或哈希发送到聊天、仓库或日志。确认恢复目标和原启用状态后再继续。
2. 只使用一个由用户在本机选定的不敏感、非根小目录；其中只选择一个小分类。目录路径、分类名、账号和凭据全部留在插件本地界面。
3. 打开目录选择器，先观察默认打开、本地搜索、筛选和选择候选时百度请求数均为零。若本轮另行明确授权从根目录进入，则只观察一次会话披露和当前一层，不选择 `/`，也不自动进入后代层。
4. 每次只由人明确启动或继续当前一层。逐轮核对 `checked_entry_count`、`list_request_count`、`next_start`、`elapsed_ms` 和精确 `stop_reason`；每轮请求的 `limit` 必须固定为 1,000，总量不得超过 20,000 条、20 次请求或 120 秒。允许的停止原因只有 `complete`、`entry-limit`、`list-request-limit`、`time-limit` 或 `user-canceled`；尚未完整时不得自动继续。
5. 在选择器中选择那一个小分类，确认界面显示所选分类的实际核验父目录，并确认选择动作没有新增列表请求、没有开始扫描或分类核验。不要点击后续核验确认。
6. 验收期间只允许 OAuth 和 `GET /rest/2.0/xpan/file?method=list`。确认文件/PDF 条目未显示、未保留，`downloaded_pdf_bytes=0`，Vault 笔记写入数为零，网盘写操作数为零；不得保存 HAR、响应正文或真实目录快照。
7. 停止后禁用插件并完全退出 Obsidian。按新的恢复授权原样恢复备份的插件数据和启用列表，核对原插件启用状态已经恢复；遇到身份不一致、并发变化或无法证明字节一致时停止，不覆盖未知对象。

本节只允许保留不含真实标识的聚合结果：本地选择器请求数、逐轮检查条目数、逐轮列表请求数、游标、耗时、上述固定停止原因、分类父目录显示是否正确、核验启动次数、PDF 下载字节数、笔记写入数、网盘写操作数和插件状态恢复结果。
